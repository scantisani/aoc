...
func Solve() uint16 {
	...
	copiedCircuit := maps.Clone(circuit)

	aSignal := circuit.FindSignal("a")
	copiedCircuit["b"] = Wire{op: bare, computed: true, signal: aSignal}

	return copiedCircuit.FindSignal("a")
}
