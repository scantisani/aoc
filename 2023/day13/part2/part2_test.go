...
func TestSmudgedReflectionSize(t *testing.T) {
	tests := []struct {
		name    string
		pattern Pattern
		want    int
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := SmudgedReflectionSize(tt.pattern); got != tt.want {
				t.Errorf("SmudgedReflectionSize() = %v, want %v", got, tt.want)
			}
		})
	}
}
...
func TestReflectionSize(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := ReflectionSize(tt.pattern, forbiddenSize); got != tt.want {
