...
func predictedValueFor(currentValues []int, extrapolations [][]int) int {
	currentFirst := currentValues[0]

	if len(extrapolations) == 1 {
		extrapolation := extrapolations[0]
		extrapolationFirst := extrapolation[0]

		return currentFirst - extrapolationFirst
	}

	return currentFirst - predictedValueFor(extrapolations[0], extrapolations[1:])
}
